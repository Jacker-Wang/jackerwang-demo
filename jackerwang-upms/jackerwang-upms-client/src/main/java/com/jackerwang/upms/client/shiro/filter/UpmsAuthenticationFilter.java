package com.jackerwang.upms.client.shiro.filter;
...
import com.jackerwang.common.util.PropertiesFileUtil;
import com.jackerwang.common.util.RedisUtil;
import com.jackerwang.upms.client.shiro.session.UpmsSessionDao;
import com.jackerwang.upms.client.util.RequestParameterUtil;
import com.jackerwang.upms.common.constant.UpmsConstant;

import redis.clients.jedis.Jedis;

/**
 * 重写authc过滤器 Created by shujackerwang on 2017/3/11.
 */
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    // 局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_ID = "jackerwang-upms-client-session-id";
    // 单点同一个code所有局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_IDS = "jackerwang-upms-client-session-ids";
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        // 判断请求类型
        String upmsType = PropertiesFileUtil.getInstance("jackerwang-upms-client").get("jackerwang.upms.type");
        session.setAttribute(UpmsConstant.UPMS_TYPE, upmsType);
        if ("client".equals(upmsType)) {
            return validateClient(request, response);
        }
        if ("server".equals(upmsType)) {
            System.out.println("验证权限 " + subject.isAuthenticated());
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        StringBuffer sso_server_url = new StringBuffer(PropertiesFileUtil.getInstance("jackerwang-upms-client").get("jackerwang.upms.sso.server.url"));
        // server需要登录
        String upmsType = PropertiesFileUtil.getInstance("jackerwang-upms-client").get("jackerwang.upms.type");
        if ("server".equals(upmsType)) {
        ...
        sso_server_url.append("/sso/index").append("?").append("appid").append("=")
                .append(PropertiesFileUtil.getInstance("jackerwang-upms-client").get("jackerwang.upms.appID"));
        // 回跳地址
